...
from smithy_aws_core.identity.chain import StandardProvider
...
def test_provider_metadata() -> None:
    ...
    assert provider.ordering.slot == StandardProvider.EC2_INSTANCE_METADATA
...
async def test_defaults_when_unconfigured(
    setup_provider: Callable[..., Awaitable[ChainSetup]],
) -> None:
    setup = await setup_provider(Ec2InstanceMetadataProvider())
    ...
    assert config.endpoint_uri.host == config._HOST_MAPPING["IPv4"]
    assert config.ec2_instance_profile_name is None
...
async def test_invalid_endpoint_raises(
    ...
    with pytest.raises(ValueError, match="endpoint URI"):
        await setup_provider(Ec2InstanceMetadataProvider())
...
async def test_empty_env_falls_back_to_profile(
    setup_provider: Callable[..., Awaitable[ChainSetup]],
    merged_config: Callable[..., MergedConfig],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # An empty environment variable must not shadow the profile value.
    monkeypatch.setenv(_ENDPOINT_MODE_ENV, "")
    config = merged_config({"default": {"ec2_metadata_service_endpoint_mode": "IPv6"}})
...
async def test_empty_env_falls_back_to_default(
    setup_provider: Callable[..., Awaitable[ChainSetup]],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # With no profile, an empty env var resolves to the default rather than raising.
    monkeypatch.setenv(_ENDPOINT_MODE_ENV, "")
