...
import pytest
from aws_credentials_http import ContainerMetadataClient, ContainerMetadataConfig
...
def test_config_custom_values() -> None:
    config = ContainerMetadataConfig(timeout=10, retries=5)
...
async def test_metadata_client_valid_host(host: str) -> None:
    response_body = json.dumps(DEFAULT_RESPONSE_DATA)
    http_client = mock_http_client_response(200, response_body.encode())
    client = ContainerMetadataClient(http_client, ContainerMetadataConfig())
...
async def test_metadata_client_https_host() -> None:
    response_body = json.dumps(DEFAULT_RESPONSE_DATA)
    http_client = mock_http_client_response(200, response_body.encode())
    client = ContainerMetadataClient(http_client, ContainerMetadataConfig())
...
async def test_metadata_client_invalid_host() -> None:
    response_body = json.dumps(DEFAULT_RESPONSE_DATA)
    http_client = mock_http_client_response(200, response_body.encode())
    client = ContainerMetadataClient(http_client, ContainerMetadataConfig(retries=0))
...
async def test_metadata_client_non_200_response() -> None:
    http_client = mock_http_client_response(404, b"not found")
    client = ContainerMetadataClient(http_client, ContainerMetadataConfig(retries=1))
...
async def test_metadata_client_invalid_json() -> None:
...
async def test_metadata_client_retries() -> None:
    http_client = AsyncMock()
    client = ContainerMetadataClient(http_client, ContainerMetadataConfig(retries=2))
