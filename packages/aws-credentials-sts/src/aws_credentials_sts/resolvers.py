...
def resolve_sts_region(
    *,
    profile_file: MergedConfig | None = None,
    profile_name: str | None = None,
) -> str:
    """Resolve the region used for STS requests."""
    profile_region = (
        profile_file.get(profile_name, _REGION)
        if profile_file is not None and profile_name is not None
        else None
    )
    return (
...
class AssumeRoleCredentialsResolver(
    ...
    """Resolve AWS credentials with an STS AssumeRole call."""
    ...
    async def get_identity(
        ...
        *,
        ...
        """Return cached credentials or call STS when they are expired."""
...
class ProfileAssumeRoleCredentialsResolver(
    ...
    """Resolve profile-based AssumeRole credentials."""

    def __init__(
        self,
        *,
        profile_name: str,
        profile_file: MergedConfig,
        region_override: str | None = None,
        http_client: HTTPClient | None = None,
    ) -> None:
        if profile_file.get_profile(profile_name) is None:
            raise AssumeRoleConfigurationError(
                f"Profile '{profile_name}' does not exist."
            )
        self._profile_name = profile_name
        self._profile_file = profile_file
        self._region = region_override or resolve_sts_region(
            profile_file=profile_file, profile_name=profile_name
        )
        self._http_client = http_client
    ...
    async def get_identity(
        ...
        *,
        ...
        """Create delegate resolver if needed, then resolve assumed credentials."""
    ...
    async def _create_assume_role_resolver(
        ...
        *,
        ...
        role_arn = profile_file.get(profile_name, _ROLE_ARN)
        if role_arn is None:
            raise AssumeRoleConfigurationError(
                f"Profile '{profile_name}' does not define role_arn"
            )

        source_profile = profile_file.get(profile_name, _SOURCE_PROFILE)
        credential_source = profile_file.get(profile_name, _CREDENTIAL_SOURCE)
        if source_profile is not None and credential_source is not None:
            ...
            print(
                f"[sts] Profile '{profile_name}' sourcing AssumeRole credentials "
                f"from source_profile '{source_profile}'."
            )
            ...
            print(
                f"[sts] Profile '{profile_name}' sourcing AssumeRole credentials "
                f"from credential_source '{credential_source}'."
            )
        ...
        print(
            f"[sts] Constructing AssumeRole resolver for profile '{profile_name}' "
            f"(role_arn='{role_arn}')."
        )
        return AssumeRoleCredentialsResolver(
            source_resolver=source_resolver,
            role_arn=role_arn,
            role_session_name=profile_file.get(profile_name, _ROLE_SESSION_NAME),
            external_id=profile_file.get(profile_name, _EXTERNAL_ID),
            duration_seconds=self._resolve_duration_seconds(profile_name),
            region=self._region,
            http_client=self._http_client,
        )

    def _resolve_duration_seconds(self, profile_name: str) -> int | None:
        raw = self._profile_file.get(profile_name, _DURATION_SECONDS)
        if raw is None:
            return None
        try:
            return int(raw)
    ...
    async def _create_resolver_from_source_profile(
        ...
        if profile_file.get_profile(source_profile_name) is None:
            raise AssumeRoleConfigurationError(
                f"Source profile '{source_profile_name}' does not exist."
            )

        if any(
            profile_file.get(source_profile_name, key) is not None
            for key in (_ACCESS_KEY_ID, _SECRET_ACCESS_KEY, _SESSION_TOKEN)
        ):
            print(
                f"[sts] Source profile '{source_profile_name}' has static "
                "credentials; terminating chain."
            )
            return self._create_static_resolver(source_profile_name)

        # If the profile is self-referencing, it must contain complete static credentials
        ...
        if profile_file.get(source_profile_name, _ROLE_ARN) is not None:
            print(
                f"[sts] Source profile '{source_profile_name}' declares a role; "
                "chaining AssumeRole."
            )
    ...
    def _create_static_resolver(
        ...
        profile_file = self._profile_file
        access_key_id = profile_file.get(profile_name, _ACCESS_KEY_ID)
        secret_access_key = profile_file.get(profile_name, _SECRET_ACCESS_KEY)
        ...
        return StaticCredentialsResolver(
            AWSCredentialsIdentity(
                ...
                session_token=profile_file.get(profile_name, _SESSION_TOKEN),
                account_id=profile_file.get(profile_name, _ACCOUNT_ID),
            )
        )
    ...
    async def _create_resolver_from_credential_source(
        ...
        if provider is None:
            raise AssumeRoleConfigurationError(
                f"No provider is installed for credential_source "
                f"'{credential_source}'. Please install '{slot.module_suggestion}'"
            )
        ...
        if not setup.resolvers:
            raise AssumeRoleConfigurationError(
                f"'{credential_source}' credential_source is not configured."
            )
