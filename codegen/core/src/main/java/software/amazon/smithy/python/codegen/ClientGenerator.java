...
final class ClientGenerator implements Runnable {
    ...
    private void writeSharedOperationInit(
        ...
        writer.writeDocs(() -> {
            ...
            var inputDocs = input.getTrait(DocumentationTrait.class)
                    .map(StringTrait::getValue)
                    .orElse(String.format("An instance of `%s`.", inputSymbolName));

            var outputDocs = customReturnDocs != null ? customReturnDocs
                    : output.getTrait(DocumentationTrait.class)
                            .map(StringTrait::getValue)
                            .orElse(String.format("An instance of `%s`.", outputSymbolName));
    ...
    private String generateEventStreamReturnDocs(
        ...
        // Subtract 12 for 3Levels of indentation (4 spaces each)
